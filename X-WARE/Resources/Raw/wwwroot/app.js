...
function callCs(p) {
    return new Promise((ok, ng) => {
        ...
        p.id = id;
        try {
            // .NET MAUI Bridge via custom scheme
            const data = encodeURIComponent(JSON.stringify(p));
            window.location.href = `sync://${p.type}?data=${data}`;
        }
...
function post(type, extra = {}) {
    try {
        const data = encodeURIComponent(JSON.stringify({ type, ...extra }));
        window.location.href = `sync://${type}?data=${data}`;
    } catch { }
}
